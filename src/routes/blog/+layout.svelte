<script lang="ts">
    ...
    const listenOpenSearchFieldKey = ({repeat, metaKey, ctrlKey, key}) => {
        if (repeat) return;
        if ((metaKey || ctrlKey) && key === "k") toggleSearchField()
    }


    const listenCloseSearchFieldKey = ({key, repeat}) => {
        if (repeat) return;
        if (key === "Escape") toggleSearchField()
    }
...
{#if showSearchField}
    <div transition:blur={{duration:200}} class="searchbg" on:click|self={toggleSearchField}>
        <div class="flex flex-col gap-y-5 mx-auto max-w-5xl overflow-hidden p-5 w-full h-full lg:h-1/2 bg-black/50 backdrop-blur-sm lg:rounded-lg lg:border lg:border-white/20">
            ...
            {:else if filteredList.length === 0 && searchText }
                <div in:fade={{delay:200,duration:200}} class="mt-5 h-20 flex justify-center items-center">
