...
<div class="relative flex flex-col gridbg rounded border border-white/20">
  <div class="absolute top-0 right-0">
    ...
    >
      ...
      {:else}
        <span class="text-white/50 hover:text-primary">
          <Icon icon="line-md:clipboard" width="26" />
        </span>
      {/if}
    </button>
  </div>
  <pre class="language-{lang} scrollbar p-5 m-0 bg-transparent"><code
      >{@html outHTML}</code
    ></pre>
</div>
