---
title: Go'da Bitwise (Bitsel) İşlemler
description: Bu yazıda Go'da Bitwise işlemlerden ve operatörlerden bahsettim.
date: '2023-09-24'
tags:
  - go
  - bitwise
...
![and-operatoru](./assets/img/and_operator.webp)
...
![and-not-operatoru](./assets/img/and_not_operator.webp)

## `|` OR (VEYA) Operatörü

![or-operatoru](./assets/img/or_operator.webp)
...
![xor-operatoru](./assets/img/xor_operator.webp)
...
![not-operatoru](./assets/img/not_operator.webp)
...
İnsanlar günlük hayatında 10'luk _(decimal veya 10 tabanı)_ sayı sistemini kullanır. Yani 0-9 arası rakamları kullanarak sayılar üretiriz. Toplamda 10 adet rakam olduğu için 10'luk _(decimal)_ sayı sistemi denir.
...
![dec-to-bin](./assets/img/dec_to_bin.webp)
...
func intToBinaryString(decimal int) (binary string) {
	if decimal < 0 {
		...
		// negatif ise pozitife çeviriyoruz.
		decimal = -decimal // -decimal * -1 = +decimal
	}

	if decimal < 2 {
		// Decimal sayımız 0 veya 1 ise bölme işlemi
...
Ben merak ettiğim için bu fonksiyonun yazımını inceledim. 64 bit uzunluğu verildiğinde aslında 63 biti parse ettiğini gördüm. Bu da fonksiyon içerisinde `bitSize - 1` şeklinde belirtilmiş. Ve bu kullanım ile maksimum bulunacak sonucu aşıp aşmadığı kontrol ediliyor.
...
![bin-to-dec](./assets/img/bin_to_dec.webp)

!> Binary sayılarda, decimal sayılardaki gibi sol tarafdaki basamak veya basamakların 0 olması sayıya değer katmaz. `0101 = 101`

Madem bu işlemin de nasıl yapıldığını öğrendiysek, kendimiz bir fonksiyon yazacak olsaydık nasıl yapacağımıza bakalım:

_(Aslında kodumuz basit fakat yorum satırlarından dolayı karmaşık duruyor 👍)_
...
`OR` operatörünü anlattığım bölümde bitleri karşılaştırıp ne sonuç verdiklerini gördük. Aslında mantıken `if-else`'de kullandığımız `||` operatörü ile aynı mantık ile çalışır.
...
![or-example](./assets/img/or_example.webp)


`&` operatörünü incelemek için `13&7` işleminin sonucuna bakalım.

```go
fmt.Println(13 & 7) // 5
```

Bu işlemde `5` sonucunun nasıl bulunduğuna bakalım.

![and-example](./assets/img/and_example.webp)

Bir de `^` - XOR (ÖZEL VEYA) için olan örneğimizi görelim.

İşlemimiz `13^7` olacak. Sürekli decimal'i binary'e dönüştürmemek için aynı sayılar üzerinden örnek verdim
...
![xor-example](./assets/img/xor_example.webp)
...
Basit olarak bahsetmek istediğim için örneğimizi `uint8` üzerinden verelim. _(Şimdi artılar eksiler birbirine karışmasın 😏)_ `uint8` tipinde bir sayının bit boyutu adından da anlaşılacağı üzere `8`'dir. `uint8`, unassigned (işaretsiz) integer olduğu için pozitif veya negatif sayı içermez. Yani dümdüz sayı saklar.
...
Bu durumda da fazla sola kaydırdığımızda soldaki bitlerimiz dışarı taşmış oluyor ve taşan bitlerimiz veri tipimizin boyutundan dolayı saklanamıyor.
Bir örnek görelim.
...
![left-shift](./assets/img/left_shift.webp)
...
![right-shift](./assets/img/right_shift.webp)

Okuduğunuz için teşekkürler. Umarım bir faydası dokunmuştur.

## Kaynaklar

- [https://pkg.go.dev/strconv#ParseInt](https://pkg.go.dev/strconv#ParseInt)
- [https://www.tutorialspoint.com/go/go_bitwise_operators.htm](https://www.tutorialspoint.com/go/go_bitwise_operators.htm)
